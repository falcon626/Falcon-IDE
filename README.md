...
`Falcon-IDE` は、クロスプラットフォームのゲーム/エンジン開発向けに設計された IDE/ツールセットの C++ ベース実装です。本リポジトリはレンダリング、エディタ拡張、スクリプトモジュールなどのサブシステムを含みます。主要な GUI やユーティリティの一部にサードパーティライブラリ（例: Dear ImGui、stb 系）を組み込んでいます。
