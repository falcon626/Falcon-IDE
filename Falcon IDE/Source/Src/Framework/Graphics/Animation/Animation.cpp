...
void Animator::ProgressTime(std::vector<ModelData::Node>& rNodes, float speed)
{
	...
	{
		// アニメーションデータによる行列補間
		animChannel.Interpolate(rNodes[animChannel.m_nodeOffset].m_mLocal, m_time);
	}
...
{
	if (m_translations.size() == Def::UIntZero)return false;

	// キー位置検索
	auto keyIdx{ static_cast<UINT>(BinarySearchNextAnimKey(m_translations, time)) };

	// 先頭のキーなら、先頭のデータを返す
	if (keyIdx == Def::UIntZero) {
	...
	// それ以外(中間の時間)なら、その時間の値を補間計算で求める
		...
		auto f{ (time - prev.m_time) / (next.m_time - prev.m_time) };
		// 補間
...
{
	if (m_rotations.size() == Def::UIntZero)return false;

	// キー位置検索
	auto keyIdx{ static_cast<UINT>(BinarySearchNextAnimKey(m_rotations, time)) };
	// 先頭のキーなら、先頭のデータを返す
	if (keyIdx == Def::UIntZero) {
...
{
	if (m_scales.size() == Def::UIntZero)return false;

	// キー位置検索
	auto keyIdx{ static_cast<UINT>(BinarySearchNextAnimKey(m_scales, time)) };
